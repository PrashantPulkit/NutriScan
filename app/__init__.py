...
def create_app():
    ...
    with open("safe_to_bulk_eat.json") as f:
        safe_data = json.load(f)
    safe_products = set(safe_data["safe_products"])

    with open("safe_concentrated.json") as f:
        concentrated_data = json.load(f)

    concentrated_products = set(concentrated_data["concentrated_products"])

    # Helper functions
    def classify_products(products):
        carbs, proteins, fats = [], [], []
        for name, data in products.items():
            c = data["carbohydrates_per_100g"]
            p = data["proteins_per_100g"]
            f = data["fat_per_100g"]
            if c > p and c > f:
                carbs.append(name)
            elif p > c and p > f:
    ...
    def fuzzy_pick(category, n, products, focus="balanced"):
        scored_products = []

        for item in category:
            data = products[item]

            kcal = data.get("energy_kcal_per_100g", 200)
            carbs = data.get("carbohydrates_per_100g", 20)
            protein = data.get("proteins_per_100g", 5)
            fat = data.get("fat_per_100g", 5)

            # Scoring Logic: Only using REAL fields
            # -------------------------------------------------
            # kcal_score: prefer around 100-150 kcal for normal foods
            kcal_score = max(0, 150 - abs(125 - kcal))

            # macronutrient scores
            if focus == "carbs":
                macro_score = carbs * 4 - (protein * 2 + fat * 2)
            elif focus == "proteins":
                macro_score = protein * 5 - (carbs * 2 + fat * 2)
            elif focus == "fats":
                macro_score = fat * 4 - (protein * 2 + carbs * 2)
            else:
                # Balanced: we want decent carbs + proteins, moderate fat
                macro_score = carbs * 2 + protein * 3 - fat

            # Final fuzzy score: macronutrient importance + kcal optimization + randomness
            final_score = macro_score + kcal_score + random.uniform(-5, 5)

            scored_products.append((final_score, item))

        # Sort by highest final fuzzy score
        scored_products.sort(reverse=True)

        # Pick top n (or fewer if not enough products)
        picked = [item for score, item in scored_products[:n]]

        return picked


    def generate_meal_plan(products, daily_calories, meals_per_day):
            
        carbs, proteins, fats = classify_products(products)
        calories_per_meal = daily_calories / meals_per_day

        meal_plan = {}

        for meal_num in range(1, meals_per_day + 1):
            carb_choices = fuzzy_pick(carbs, 2,products)
            protein_choices = fuzzy_pick(proteins, 1,products)
            fat_choices = fuzzy_pick(fats, 1,products)

            meal_items = carb_choices + protein_choices + fat_choices
            ...
            for item in meal_items:
                data = products[item]
                kcal = data["energy_kcal_per_100g"]
                quantity = random.randint(50, 150)
                item_calories = kcal * quantity / 100
                total_calories += item_calories

                meal[item] = {
                    "quantity_g": quantity,
                    "estimated_kcal": round(item_calories, 1)
                }

            adjustment_factor = calories_per_meal / total_calories
            for item in meal:
                meal[item]["quantity_g"] = round(meal[item]["quantity_g"] * adjustment_factor)
                meal[item]["estimated_kcal"] = round(products[item]["energy_kcal_per_100g"] * meal[item]["quantity_g"] / 100, 1)

            meal_plan[f"Meal {meal_num}"] = meal
    ...
    def dietform():
        if request.method == "POST":
            ...
            meal_plan = generate_meal_plan(products, daily_calories, meals_per_day)
            return render_template("meal_plan.html", meal_plan=meal_plan, daily_calories=daily_calories, meals_per_day=meals_per_day)

        return '''
            <form method="POST">
                <label>Daily Calories: </label><input type="number" name="daily_calories" value="2000"><br><br>
                <label>Meals per Day: </label><input type="number" name="meals_per_day" value="3"><br><br>
